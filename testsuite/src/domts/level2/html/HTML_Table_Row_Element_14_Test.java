...
*/
class HTML_Table_Row_Element_14_Test extends domts.DOMTestCase {
 @BeforeEach
 void setup() {
 ...
 @Test
 @DisplayName("http://www.w3.org/2001/DOM_Test_Suite/level2/html/HTMLTableRowElement14")
 void run() throws Throwable {
  ...
  assertEquals("Accountant", value, "value1Link");
  ((HTMLTableRowElement) /*Node */testNode).deleteCell(2);
  testNode = nodeList.item(3);
  cellsnodeList = ((HTMLTableRowElement) /*Node */testNode).getCells();
  vcells = (int) cellsnodeList.getLength();
  ...
  cellNode = trNode.getFirstChild();
  value = cellNode.getNodeValue();
  assertEquals("56,000", value, "value2Link");
 }
}